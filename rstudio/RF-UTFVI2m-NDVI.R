...
#era5 = "era5_2020_cat"
ano = '2000'
era5 = paste('era5_modis_2m',ano,sep="_")
era5
...
data <- subset(data, select = c(RH, windspeed, windquad,NDVI, UHIp2m))
data
...
rf <- randomForest(x = treino[,-5],
                   ...
                   keep.forest = T)
rf
title = paste('Importâncias das Variáveis Micro-Climáticas para o Fenômeno ICU do ano',ano,sep=" ")
...
#REGRAS
target = UHIp2m ~ RH + windspeed + windquad + NDVI
fit <- rpart(target, data = data)
rpart.rules(fit,cover=TRUE)

#UHIP rate was greater for ICU
rpart.rules(rpart(UHIp2m ~ NDVI + windquad + windspeed, data = data))

#windquad rate was greater for Relative Humidity
rpart.rules(rpart(windquad ~ RH + NDVI + UHIp2m + windspeed, data = data))

#windquad rate was greater for Relative Humidity
rpart.rules(rpart(windspeed ~ RH + NDVI + UHIp2m + windquad, data = data))
