...
#era5 = "era5_2020_cat"
ano = '2000'
era5 = paste('era5_modis',ano,sep="_")
print(era5)
...
data <- subset(data, select = c(RH, windspeed, windquad,NDVI, UHIp))
data
...
#REGRAS
target = UHIp ~ RH + windspeed + windquad + NDVI
fit <- rpart(target, data = data)
rpart.rules(fit,cover=TRUE)

#UHIP rate was greater for ICU
rpart.rules(rpart(UHIp ~ NDVI + windquad + windspeed, data = data))

#windquad rate was greater for Relative Humidity
rpart.rules(rpart(windquad ~ RH + NDVI + UHIp + windspeed, data = data))

#windquad rate was greater for Relative Humidity
rpart.rules(rpart(windspeed ~ RH + NDVI + UHIp + windquad, data = data))
